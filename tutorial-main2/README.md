...
To try the genetic-algorithm tuning loop that uses a local Ollama model:
```
uv run agentbeats-run scenarios/ga_tuning/scenario.toml --show-logs
```
(Requires an Ollama server with `llama3.2:latest` pulled and `OLLAMA_BASE_URL` set; see `sample.env`.)
