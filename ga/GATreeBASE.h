...
/** @brief base tree class from which template trees are derived.
...
/** @brief base class for iterators for the tree objects.
...
of speed we don't do any error checking.
 ...
 @li current, root, next, prev, parent, child, eldest, youngest, warp
...
public:
    ...
    int depth()
    {
        return(node ? depth(node) : 0);
    }
    int depth(GANodeBASE * c);
    int nchildren()
    {
        return(node ? nchildren(node) : 0);
    }
    int nchildren(GANodeBASE * c);
    int nsiblings()
    {
        return(node ? nsiblings(node) : 0);
    }
