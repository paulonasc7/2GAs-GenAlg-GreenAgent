...
def test_parse_params_strips_code_fence():
    agent = Agent()
    raw = """```json
{
  "crossover_type": "onepoint",
  "selector_type": "roulette",
...
```"""
    parsed = agent._parse_params(raw)
    assert parsed["crossover_type"] == "onepoint"
    assert parsed["selector_type"] == "roulette"


def test_map_params_basic():
    agent = Agent()
    params = {
        "crossover_type": "twopoint",
        "selector_type": "tournament",
    ...
    mapped = agent._map_params(params)
    assert mapped["crossover"] == "twopoint"
    assert mapped["selector"] == "tournament"
    assert mapped["pcross"] == 0.8
    assert mapped["pmut"] == 0.02
    assert mapped["popsize"] == 120
    assert mapped["generations"] == 250
    assert mapped["elitism"] is False
